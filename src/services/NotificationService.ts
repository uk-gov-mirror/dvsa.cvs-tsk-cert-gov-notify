...
class NotificationService {
	 ...
	 */
	public async sendNotification(notifyPartialParams: IPartialParams, templateId: string, filename: string | null = null) {
		if (!notifyPartialParams?.shouldEmail || notifyPartialParams?.shouldEmail === 'true') {
			const emailDetails = {
				personalisation: {
					...notifyPartialParams.personalisation,
					link_to_document:
						this.notifyClient.prepareUpload(notifyPartialParams.fileData, {
